/**
 * @file ProxGVIMPPlanarRobotSDF.h
...
class ProxGVIMPPlanarRobotSDF{
    using SDFPR = gpmp2::ObstaclePlanarSDFFactor<Robot>;
    using ProxGVIFactorizedPlanarSDFRobot = ProxGVIFactorizedPlanarSDF<Robot>;

public:
    virtual ~ProxGVIMPPlanarRobotSDF(){}

    ProxGVIMPPlanarRobotSDF(){}

    ProxGVIMPPlanarRobotSDF(GVIMPParams& params):
    ...
    std::tuple<Eigen::VectorXd, gvi::SpMat> run_optimization_return(const GVIMPParams& params, bool verbose=true){
        ...
        for (int i = 0; i < n_states; i++) {
            ...
            if (i==0 || i==n_states-1){
                ...
                vec_factors.emplace_back(new ProxGVIFactorizedPlanarSDFRobot{dim_conf, 
                                                                            dim_state, 
                                                                            params.GH_degree(),
                                                                            cost_sdf_Robot, 
        ...
        /// The joint optimizer
        gvi::ProxGVIGH<gvi::GVIFactorizedBase> optimizer{vec_factors, 
                                                        dim_state, 
                                                        n_states, 
                                                        params.max_iter(), 
                                                        params.temperature(), 
...
protected: 
    ...
    std::shared_ptr<gvi::ProxGVIGH<gvi::GVIFactorizedBase>> _p_opt;
