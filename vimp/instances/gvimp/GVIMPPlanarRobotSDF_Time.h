...
class GVIMPPlanarRobotSDF_Time{
    using SDFPR = gpmp2::ObstaclePlanarSDFFactor<Robot>;
    using NGDFactorizedPlanarSDFRobot = NGDFactorizedPlanarSDF<Robot>;

public:
    virtual ~GVIMPPlanarRobotSDF_Time(){}

    GVIMPPlanarRobotSDF_Time(){}

    GVIMPPlanarRobotSDF_Time(GVIMPParams& params):
    ...
    std::tuple<Eigen::VectorXd, gvi::SpMat> run_optimization_return(const GVIMPParams& params, bool verbose=true){
        ...
        for (int i = 1; i < n_states-1; i++) {
            ...
            NGDFactorizedPlanarSDFRobot* factor = new NGDFactorizedPlanarSDFRobot{dim_conf, 
                                                                                dim_state, 
                                                                                params.GH_degree(),
                                                                                cost_sdf_Robot, 
            ...
            #pragma omp critical
            {
                vec_factors.emplace_back(factor);
            }  
        }
