...
#include "instances/gvimp/ProxGVIMPPlanarPRSDF.h"
...
int main(int argc, char* argv[]){
    ...
    if (argc == 1){
        num_exp = 4;
        std::string config_file{source_root+"/configs/vimp/planar_pR_map2_proxGVI.xml"};
        GVIMPRunner<ProxGVIMPPlanarPRSDF> runner(nx, nu, num_exp, config_file);
    ...
    // arguments: i_exp, params:(i_exp, eps, eps_sdf, speed, nt, sig0, sigT, eta, stop_err, max_iter, cost_sig)
        ...
        GVIMPRunner<ProxGVIMPPlanarPRSDF> runner(nx, nu, num_exp, config_abs_path);
        runner.read_config(params);
        runner.run();
        return 0;
    }
