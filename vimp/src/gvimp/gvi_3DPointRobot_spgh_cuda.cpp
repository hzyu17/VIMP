...
#include "instances_cuda/gvimp/GVIMP3DPointRobotSDF.h"
...
int main(int argc, char* argv[]){
    ...
    if (argc == 1){
        ...
        GVIMPRunner<GVIMP3DPointRobotSDF> runner(nx, nu, num_exp, config_file);
    ...
    // arguments: i_exp, params:(i_exp, eps, eps_sdf, speed, nt, sig0, sigT, eta, stop_err, max_iter, cost_sig)
        ...
        GVIMPRunner<GVIMP3DPointRobotSDF> runner(nx, nu, num_exp, config_abs_path);
        runner.read_config(params);
        runner.run();
        return 0;
    }
