...
#include "instances_cuda/proxkl/ProxKLPR2ArmSDF.h"
...
int main(int argc, char* argv[]){
    ...
    if (argc == 1){
        std::string config_file{source_root+"/configs/proxkl/pr2_bookshelf.xml"};
        GVIMPRunner7D<ProxKLPR2ArmSDF> runner(num_exp, config_file);
        ...
        std::string config_file{source_root+"/configs/proxkl/wam_map1_new.xml"};
        GVIMPRunner7D<ProxKLPR2ArmSDF> runner(num_exp, config_file);
