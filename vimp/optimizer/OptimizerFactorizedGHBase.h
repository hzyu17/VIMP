...
namespace vimp{
    ...
    public:
        ...
        virtual void calculate_partial_V(){
            ...
            _gh->update_integrand(_func_Vmu);
            Vdmu = _gh->Integrate();

            Vdmu = _precision * Vdmu;

            /// Integrate for E_q{phi(x)}
            _gh->update_integrand(_func_phi);
            double E_phi = _gh->Integrate()(0, 0);
            
            /// Integrate for partial V^2 / ddmu_ 
            _gh->update_integrand(_func_Vmumu);
            MatrixXd E_xxphi{_gh->Integrate()};
