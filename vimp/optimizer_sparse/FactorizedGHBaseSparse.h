/**
 * @file FactorizedGHBaseSparse.h
 * @author Hongzhe Yu (hyu419@gatech.edu)
 * @brief The base class for marginal optimizer, using sparse matrices.
 * @version 0.1
 * @date 2023-01-07
 * 
...
namespace vimp{
    ...
    class VIMPFactorizedGHBaseSparse{
    public:
        
        /**
         * @brief Default Constructor
         */
        FactorizedGHBaseSparse(){}

        /**
         * @brief Construct a new VIMPOptimizerFactorizedBase object
         * 
         * @param size Size of the factor
         * @param BlockIndex The index telling the corresponding factor.
         */
        FactorizedGHBaseSparse(int size, const BlockIndex& block_indx):
                _dim{size},
                _mu{Vector::Zero(_dim)},
                _covariance{SpMat(_dim, _dim)},
                _precision{SpMat(_dim, _dim)},
                _dprecision{SpMat(_dim, _dim)},
                _Vdmu{Vector::Zero(_dim)},
                _Vddmu{SpMat(_dim, _dim)}
                {
                    _covariance.setIdentity();
                    _precision.setIdentity();
                    _dprecision.setZero();
                    _Vddmu.setZero();
                }
