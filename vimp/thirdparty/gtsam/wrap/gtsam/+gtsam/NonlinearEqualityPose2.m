...
classdef NonlinearEqualityPose2 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualityPose2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2017, varargin{2});
        end
        base_ptr = gtsam_wrapper(2016, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Pose2')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2018, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Pose2') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2019, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2020, obj.ptr_gtsamNonlinearEqualityPose2);
    end
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(2030, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(2036, varargin{:});
      else
