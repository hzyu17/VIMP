    ...
    def _on_timer(self, event):
        with self._lock:
            ...
            for name, buf in self._buffers.items():
                ...
                # update MoveIt!
                # remove + add ensures moving objects get repositioned
                # self._scene.remove_world_object(name)
                print("name: ", name)
                print("out.pose: ", out.pose)
                print("size: ", self._sizes[name])
    ...
    body_frames:
      - frame_id: B1
        ...
        box:
          ...
          center: [cx, cy, cz]
          orientation: [qx, qy, qz, qw]
          size: [sx, sy, sz]        # <— you’ll need to add this field!
...
if __name__ == "__main__":
    cfg_path = Path(__file__).parent.parent / "hardware_experiment" / "config" / "body_frame.yaml"
