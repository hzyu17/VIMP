...
## Add artificial random noise and the collision checking distances of the baseline trajectories
...
```
python vimp/scripts/hardware_experiment/hardware_bodyframe_publisher.py
```
after running the script, hit ENTER key for multiple (~10) times, the poses will be saved in Data/poses.yaml. After that, run the following script to compute the mean of the default (nominal) pose:
```
read_default_poses.py
``` 
...
python vimp/scripts/hardware_experiment/hardware_bodyframe_publisher.py
```
Before running, change the output file in the script hardware_bodyframe_publisher.py to differentiate from Data/poses.yaml.
