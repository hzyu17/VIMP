...
#include "instances_cuda/FactorizedGVIPlanarNGD_Cuda.h"
...
class GVIMPPlanarRobotSDF_Cuda{
    using SDFPR = gpmp2::ObstaclePlanarSDFFactor<Robot>;
    using NGDFactorizedPlanarSDFRobot_Cuda = NGDFactorizedPlanarSDF_Cuda<Robot>;

public:
    virtual ~GVIMPPlanarRobotSDF_Cuda(){}
    ...
    std::tuple<Eigen::VectorXd, gvi::SpMat> run_optimization_return(const GVIMPParams& params, bool verbose=true){
        ...
        for (int i = 0; i < n_states; i++) {
            ...
            if (i==0 || i==n_states-1){
                ...
                vec_factors.emplace_back(new NGDFactorizedPlanarSDFRobot_Cuda{dim_conf, 
                                                                        dim_state, 
                                                                        params.GH_degree(),
                                                                        cost_sdf_Robot, 
