...
classdef DummyFactor < gtsam.NonlinearFactor
  ...
  methods
    function obj = DummyFactor(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_unstable_wrapper(287, varargin{2});
        end
        base_ptr = gtsam_unstable_wrapper(286, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'numeric') && isa(varargin{4},'numeric')
        [ my_ptr, base_ptr ] = gtsam_unstable_wrapper(288, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gtsam_unstable_wrapper(289, obj.ptr_gtsamDummyFactor);
    end
