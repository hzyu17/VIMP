...
classdef Gaussian < gtsam.noiseModel.Base
  ...
  methods
    function obj = Gaussian(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(650, varargin{2});
        end
        base_ptr = gtsam_wrapper(649, my_ptr);
      else
    ...
    function delete(obj)
      gtsam_wrapper(651, obj.ptr_gtsamnoiseModelGaussian);
    end
  ...
  methods(Static = true)
    function varargout = Covariance(varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(655, varargin{:});
      else
