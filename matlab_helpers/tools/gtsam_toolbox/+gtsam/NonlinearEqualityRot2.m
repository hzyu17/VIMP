...
classdef NonlinearEqualityRot2 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualityRot2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1975, varargin{2});
        end
        base_ptr = gtsam_wrapper(1974, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Rot2')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1976, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Rot2') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1977, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1978, obj.ptr_gtsamNonlinearEqualityRot2);
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(1994, varargin{:});
      else
