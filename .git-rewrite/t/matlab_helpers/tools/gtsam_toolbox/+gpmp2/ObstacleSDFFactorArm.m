...
classdef ObstacleSDFFactorArm < gtsam.NoiseModelFactor
  ...
  methods
    function obj = ObstacleSDFFactorArm(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(222, varargin{2});
        end
        base_ptr = gpmp2_wrapper(221, my_ptr);
      elseif nargin == 5 && isa(varargin{1},'numeric') && isa(varargin{2},'gpmp2.ArmModel') && isa(varargin{3},'gpmp2.SignedDistanceField') && isa(varargin{4},'double') && isa(varargin{5},'double')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(223, varargin{1}, varargin{2}, varargin{3}, varargin{4}, varargin{5});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(224, obj.ptr_gpmp2ObstacleSDFFactorArm);
    end
    ...
    function varargout = evaluateError(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'double') && size(varargin{1},2)==1
        varargout{1} = gpmp2_wrapper(225, this, varargin{:});
      else
