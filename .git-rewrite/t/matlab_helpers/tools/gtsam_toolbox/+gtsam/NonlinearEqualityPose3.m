...
classdef NonlinearEqualityPose3 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualityPose3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2038, varargin{2});
        end
        base_ptr = gtsam_wrapper(2037, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Pose3')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2039, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Pose3') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2040, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2041, obj.ptr_gtsamNonlinearEqualityPose3);
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(2057, varargin{:});
      else
