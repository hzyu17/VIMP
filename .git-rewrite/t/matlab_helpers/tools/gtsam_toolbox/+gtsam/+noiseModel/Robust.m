...
classdef Robust < gtsam.noiseModel.Base
  ...
  methods
    function obj = Robust(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(725, varargin{2});
        end
        base_ptr = gtsam_wrapper(724, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'gtsam.noiseModel.mEstimator.Base') && isa(varargin{2},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(726, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(727, obj.ptr_gtsamnoiseModelRobust);
    end
  ...
  methods(Static = true)
    function varargout = Create(varargin)
      ...
      if length(varargin) == 2 && isa(varargin{1},'gtsam.noiseModel.mEstimator.Base') && isa(varargin{2},'gtsam.noiseModel.Base')
        varargout{1} = gtsam_wrapper(729, varargin{:});
      else
