...
classdef Fair < gtsam.noiseModel.mEstimator.Base
  ...
  methods
    function obj = Fair(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(707, varargin{2});
        end
        base_ptr = gtsam_wrapper(706, my_ptr);
      elseif nargin == 1 && isa(varargin{1},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(708, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(709, obj.ptr_gtsamnoiseModelmEstimatorFair);
    end
  ...
  methods(Static = true)
    function varargout = Create(varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      varargout{1} = gtsam_wrapper(711, varargin{:});
    end
