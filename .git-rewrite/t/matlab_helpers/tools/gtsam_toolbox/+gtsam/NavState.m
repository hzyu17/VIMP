  ...
  methods
    function obj = NavState(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(2207, my_ptr);
      elseif nargin == 0
        my_ptr = gtsam_wrapper(2208);
      elseif nargin == 3 && isa(varargin{1},'gtsam.Rot3') && isa(varargin{2},'gtsam.Point3') && isa(varargin{3},'double')
        my_ptr = gtsam_wrapper(2209, varargin{1}, varargin{2}, varargin{3});
      elseif nargin == 2 && isa(varargin{1},'gtsam.Pose3') && isa(varargin{2},'double')
        my_ptr = gtsam_wrapper(2210, varargin{1}, varargin{2});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2211, obj.ptr_gtsamNavState);
    end
