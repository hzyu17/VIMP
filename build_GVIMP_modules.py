...
print("Installing CUDA. Note that this requires a compatible nvidia GPU.")
time.sleep(2)
os.system("mkdir dependencies")
print("Assuming an x86_64 CPU architecture.")
time.sleep(0.5)
os.chdir(DEP_DIR)
...
print("Updating GPU capability in CMake")
time.sleep(2)
for line in file:
...
for line in GVI_file_contents:
    ...
    if "set(CUDA_ARCH" in line:
        replace = line.replace("sm_81", arch_sm)
        GVI_file_contents[line_num] = replace
    line_num += 1
GVI_file.seek(0)
GVI_file.truncate(0)
