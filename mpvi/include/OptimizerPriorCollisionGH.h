/**
 * @file OptimizerPriorCollisionGH.h
 ...
 */
 ...
 /// Joint optimizer considering the single state prior and collision cost.
...
#include "OptimizerFactorizedPriorCollisionGH.h"
...
class VIMPOptimizerPriorColGH{
    using FactorizedOptimizer = OptimizerFactorizedPriorColGH<Function, PriorClass, CollisionClass, Args ...>;

public:
    VIMPOptimizerGH(const int& dimension, const int& sub_dim, const vector<Function>& _vec_function,
                                   const vector<Function>& _vec_function,
                                   const vector<PriorClass>& _vec_prior,
                                   const vector<CollisionClass>& _vec_collision,
                                   const vector<MatrixXd>& _vec_Pks):
                                   dim{dimension},
                                   sub_dim{sub_dim},
                                   num_sub_vars{static_cast<int>(_vec_Pks.size())},
                                   vec_cost_function_{_vec_function},
                                   vec_prior_{_vec_prior},
                                   vec_collision_{_vec_collision},
                                   vec_Pks_{_vec_Pks},
        ...
        for (int i=0; i<num_sub_vars; i++){
            ...
            FactorizedOptimizer optimizer_k{sub_dim, vec_cost_function_[i]};
...
protected:
    ...
    const vector<PriorClass> vec_prior_,
    const vector<CollisionClass> vec_collision_,
